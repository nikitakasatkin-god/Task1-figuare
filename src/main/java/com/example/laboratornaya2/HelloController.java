...
public class HelloController {
    ...
    private void onCombineButtonClick() {
        if (!selectedShapes.isEmpty()) {
            ...
            saveState();
            redrawCanvas();
            currentMode = Mode.MOVE; // После объединения переходим в режим перемещения
        }
    }
        ...
        if (!selectedShapes.isEmpty()) {
            ...
            selectedShapes.forEach(shape -> {
                if (shape instanceof CompositeShape) {
                    ...
                    gc.strokeRect(
                            group.getMinX() - 5, group.getMinY() - 5,
                            group.getMaxX() - group.getMinX() + 10,
                            group.getMaxY() - group.getMinY() + 10
                    );
                } else {
                    gc.strokeRect(
                            shape.getX() - 10, shape.getY() - 10, 20, 20
                    );
                }
            });
            gc.setLineDashes(null);
